    ...
    <div class="container">
        <div class="delivery-card">
                        ...
                        <div>
                            ...
                            <p class="header-subtitle">
                                ...
                                <span class="header-badge">
                                    <i class="fas fa-calendar-alt"></i> {{ $shipment->created_at->format('d M Y, h:i A') }}
                                </span>
                            </p>
                        </div>
                    </div>
            ...
            <div class="info-grid" style="padding-bottom: 0;">
                <div class="info-card" style="background: linear-gradient(135deg, #f8fafc, #ffffff); text-align: center;">
                    @php
                        $statusColors = [
                    ...
                    <div class="status-badge {{ $statusClass }}" style="display: inline-flex;">
                        <i class="fas fa-{{ $shipment->status == 'pending' ? 'clock' : ($shipment->status == 'in_transit' ? 'truck' : ($shipment->status == 'delivered' ? 'check-circle' : 'box')) }}"></i>
                        {{ ucfirst(str_replace('_', ' ', $shipment->status)) }}
                    </div>
                    @if($shipment->estimated_delivery_date)
                        <p class="mt-2 mb-0 text-muted">
                            <i class="fas fa-calendar-week"></i> Est. Delivery: {{ $shipment->estimated_delivery_date->format('d M Y') }}
                        </p>
                    @endif
                </div>
            </div>
            ...
            <div class="info-grid">
                ...
                <div class="info-card">
                    ...
                    <div class="info-row">
                        ...
                        <span class="info-value">{{ $shipment->shipping_address }}, {{ $shipment->city }}, {{ $shipment->state }} - {{ $shipment->pincode }}</span>
                    </div>
            ...
            <div class="action-section">
                @if($shipment->status == 'pending')
                    ...
                    <div class="action-buttons">
                        <form action="{{ route('agent.delivery.start', $shipment->id) }}" method="POST" style="width: 100%;">
                            @csrf
                            <button type="submit" class="btn btn-success w-100" onclick="return confirm('Start delivery for shipment #{{ $shipment->shipment_number }}?')">
                ...
                @elseif(in_array($shipment->status, ['picked', 'in_transit', 'out_for_delivery']))
                    <div class="alert alert-info">
                        ...
                        <span>Delivery in progress. Click "Track & Complete" to view live location and complete delivery.</span>
                    </div>
                    <div class="action-buttons">
                ...
                @elseif($shipment->status == 'delivered')
                    <div class="alert alert-success" style="background: #d1fae5; border-left-color: #10b981;">
                        ...
                        <span>Delivered successfully on {{ $shipment->actual_delivery_date?->format('d M Y, h:i A') }}</span>
                    </div>
                    <div class="action-buttons">
            ...
            <div class="timeline-section">
                <div class="card-header-custom" style="margin-bottom: 1.5rem;">
                    <div class="card-icon" style="width: 40px; height: 40px;"><i class="fas fa-history"></i></div>
                ...
                <div class="timeline">
                    ...
                    <div class="timeline-item">
                        <div class="timeline-status">
                            <i class="fas fa-{{ $track->status == 'delivered' ? 'check-circle' : ($track->status == 'in_transit' ? 'truck' : 'clock') }}"></i>
                            {{ ucfirst(str_replace('_', ' ', $track->status)) }}
                        </div>
                        <div class="timeline-time">
                            <i class="fas fa-calendar-alt"></i> {{ $track->tracked_at->format('d M Y, h:i A') }}
                        </div>
                        @if($track->location)
